...
/// LFO, envelope, etc.) using a `Param`. This makes it extremely flexible for
/// creating time-varying timbres.
///
/// The duty cycle parameter value is expected to be in the range [-1.0, 1.0]
/// and will be automatically scaled and clamped to [0.0, 1.0].
pub struct PulseOscillator {
    ...
    /// Sample rate in Hz
    sample_rate: f64,
    /// Duty cycle parameter (0.0 to 1.0) - fraction of cycle where output is high
    duty_cycle: Param,
}
...
impl PulseOscillator {
    ...
    /// * `duty_cycle` - Parameter for duty cycle (fixed value or modulated signal)
    ///
    /// # Examples
    ///
    /// ```
    /// use earworm::{PulseOscillator, Signal, SineOscillator};
    ///
    /// // Create a pulse wave with fixed 25% duty cycle
    /// let mut pulse = PulseOscillator::new(440.0, 44100.0, 0.25.into());
    ...
    /// let mut pulse = PulseOscillator::new(440.0, 44100.0, lfo.into());
    /// let sample = pulse.next_sample();
    /// ```
    pub fn new(frequency: f64, sample_rate: f64, duty_cycle: Param) -> Self {
...
impl AudioSignal for PulseOscillator {
    fn sample_rate(&self) -> f64 {
        self.sample_rate
    }
}

impl Signal for PulseOscillator {
    fn next_sample(&mut self) -> f64 {
        // Get duty cycle from parameter and scale from [-1, 1] to [0, 1]
...
mod tests {
    ...
    fn test_oscillator_creation() {
        let osc = PulseOscillator::new(440.0, 44100.0, 0.5.into());
        assert_eq!(osc.frequency(), 440.0);
    }

    #[test]
    fn test_frequency_change() {
        let mut osc = PulseOscillator::new(440.0, 44100.0, 0.5.into());
    ...
    fn test_waveform_shape_50_percent() {
        ...
        let mut osc = PulseOscillator::new(1.0, 100.0, 0.5.into());
    ...
    fn test_waveform_shape_25_percent() {
        ...
        let mut osc = PulseOscillator::new(1.0, 100.0, 0.25.into());
    ...
    fn test_phase_wrapping() {
        let mut osc = PulseOscillator::new(1000.0, 44100.0, 0.5.into());
    ...
    fn test_zero_frequency() {
        let mut osc = PulseOscillator::new(0.0, 44100.0, 0.5.into());
    ...
    fn test_modulating_duty_cycle() {
        ...
        let mut osc = PulseOscillator::new(10.0, 100.0, lfo.into());
    ...
    fn test_duty_cycle_scaling() {
        ...
        let mut osc = PulseOscillator::new(1.0, 100.0, (-1.0).into()); // Should map to 0.0
        let sample1 = osc.next_sample();
        // With duty cycle 0.0, output should always be -1.0
        assert_eq!(sample1, -1.0);

        let mut osc = PulseOscillator::new(1.0, 100.0, 1.0.into()); // Should map to 1.0
        let sample2 = osc.next_sample();
